...
>
> ### 🔴 `<li>` 안 이미지도 포함합니다 (2026-08-02 추가 판정)
>
> 잔여 5장(`ux-trends-…-1` 4장 + `mongodb-local` 1장)이 전부 `<li>` 안이라 규칙 밖이었습니다. **포함시킵니다.**
>
> `mongodb-local` 한 글에서 **11장은 매트, 1장은 무매트**로 보이는 상태인데, 이건 직전 판정에서 *"가장 나쁜 결과"*로 지목한 바로 그 불일치입니다. 그리고 매트의 존재 이유(다크모드 흰 스크린샷 눈부심)는 **부모 요소가 무엇이든 똑같이 성립**합니다.
>
> **규칙을 부모 태그가 아니라 배치 수준으로 일반화합니다.**
>
> > **본문의 블록 수준 이미지는 부모가 `<p>`든 `<li>`든 전부 `<figure>`로 승격한다. 인라인 이미지(텍스트 사이에 낀 것)만 예외.**
>
> 이렇게 두면 `<blockquote>`·`<td>` 같은 다른 부모가 나와도 규칙을 또 고칠 필요가 없습니다.
>
> - `<figure>`를 `<li>` 안에 두는 것은 HTML 사양상 유효합니다
> - **리스트 마커 정렬이 깨지지 않게** `figure` 상하 마진을 리스트 문맥에서 축소하세요(본문 기본값보다 작게)
> - 캡션 규칙(`<br>` + 텍스트 → `<figcaption>`)은 동일하게 적용
> - 목표 커버리지 **113/113 (100%)**
>
> ### 캡션 마크업 찌꺼기 (`client-side-ai` 12건) — 원문 수정 권고
>
> `<code><font color="#7f7f7f"></code>` 형태가 캡션에 노출됩니다. 원문 md가 백틱으로 죽은 `<font>` 태그를 감싼 탓이며 **회귀가 아닙니다.** 다만 본문 안에 있을 때보다 **캡션 자리로 오면서 훨씬 눈에 띕니다** — 독자에게는 이미지 밑에 정체불명의 코드 조각이 붙은 것으로 보입니다.
>
> **판정: 원문 md 1편을 고치는 쪽을 권합니다.**
>
> - `41편 무수정` 원칙은 **프론트매터·파일명·이미지 경로 구조를 건드리지 않는다**는 뜻이지, 명백한 마크업 오류까지 보존하라는 뜻이 아닙니다. 1편 12줄은 마이그레이션이 아니라 **오탈자 수정 수준**입니다
> - 렌더 단계 특수 처리는 **이 한 글을 위해 파이프라인에 예외를 심는 것**이라 더 비쌉니다. 앞서 이미지에서 "내용 판별 로직을 만들지 말라"고 한 것과 같은 이유입니다
>
> **대안 (원문을 못 고칠 경우)**: `figcaption` **안에서만** 내용이 빈 `<font>` 태그를 언랩. 적용 범위를 캡션으로 한정해 본문 렌더에 영향이 없게 하세요.
> ```
>
> | 입력 `<p>` | 출력 |
> |---|---|
