...
>
> ### 🔴 「줄 끝 이미지」 — 규칙을 넓히는 게 아니라 **읽는 법을 바로잡습니다** (2026-08-02)
>
> `2025-03-13` 233·235행 `- ( PC 버전의 네이버) ![img]` 2장이 인라인으로 분류돼 111/113에서 멈췄습니다.
>
> **확정 규칙을 다시 읽어 보세요.**
>
> > *"이미지 **앞뒤가 모두** 텍스트인 경우(인라인 배지 등)**에만** 승격하지 않는다."*
>
> 이 케이스는 **앞에만 텍스트가 있고 뒤에는 없습니다.** 규칙대로면 **이미 승격 대상**입니다. 구현이 "같은 줄에 텍스트가 있으면 인라인"이라는 **더 좁은 해석**을 적용한 것이지, 규칙이 이 케이스를 배제한 적이 없습니다.
>
> → **규칙 확장이 아니라 해석 정정입니다. 목표는 113/113 그대로입니다.**
>
> **경계를 오해 없이 다시 씁니다**
>
> > **이미지 양옆이 **모두** 텍스트일 때만 인라인이다. 한쪽만 텍스트면 그 텍스트는 캡션이고, 이미지는 블록으로 승격한다.**
>
> | 배치 | 처리 |
> |---|---|
> | `텍스트 <img> 텍스트` | 인라인 — 승격 안 함 |
> | `텍스트 <img>` (뒤에 없음) | 승격. 앞 텍스트가 **`<figcaption>`**(figure의 **첫 자식**) |
> | `<img> 텍스트` (앞에 없음) | 승격. 뒤 텍스트가 **`<figcaption>`**(figure의 **마지막 자식**) |
> | `<img>` 단독 | 승격. 캡션 없음 |
>
> `<figcaption>`은 `<figure>`의 **첫 자식 또는 마지막 자식** 어느 쪽이든 유효합니다(HTML 사양). 앞에 오는 라벨을 굳이 뒤로 옮기지 마세요 — **원문의 읽는 순서를 보존**하는 편이 낫습니다.
>
> ### 원문 md 잔여 2건 — **1건만 고치고 1건은 두세요**
>
> | 위치 | 판정 |
> |---|---|
> | 105행 `<htmll>` 오타 | ✅ **고치세요.** 1글자 삭제입니다. **코드 예제 안이라면 더더욱** — 틀린 마크업을 가르치는 셈이 됩니다 |
> | 72행 깨진 코드펜스 | ❌ **손대지 마세요.** frontend 판단이 옳습니다 |
>
> **깨진 코드펜스를 두는 이유**: ① 이미 코드 스팬 안이라 **파싱이 깨지지 않고 렌더 결함이 없습니다** ② 고치려면 **블록을 재작성**해야 하는데, 그건 오탈자 수정이 아니라 **콘텐츠 편집**입니다.
>
> 앞서 세운 경계가 그대로 적용됩니다 — *"`41편 무수정`은 프론트매터·파일명·이미지 경로 구조를 건드리지 않는다는 뜻이고, 명백한 오탈자 수정은 그 예외다."* **재작성은 그 예외의 범위를 넘습니다.** 원저자만 의도를 알 수 있으므로 사용자에게 알리고 판단을 맡기세요.
> ```
>
> | 입력 `<p>` | 출력 |
> |---|---|
...
- **1회성 태그는 링크하지 않습니다** (product.md §⑦). 링크 없는 태그는 `<span>`으로 렌더하고 호버 커서를 바꾸지 마세요
  - 🔴 **2026-08-13 실측 정정**: 1회성은 ~~37종~~ **35종**입니다(전체 62종 · 인덱스 27종). 소스 프론트매터가 이미 정규형이라 원문 = 정규화 후입니다 — 근거 `handoff-step6-tags-archive.md` §3-5. **수치를 여기 다시 박아 넣지 말고 `tags-data.json`에서 세십시오**
