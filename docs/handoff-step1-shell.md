...
| 6 | 워드마크 폭 (신규) | `Seobisback.log` 22px ≈ 150px | `섭우.log`(약 80px)보다 깁니다. 모바일 390px에서 아이콘 3개(132px) + 패딩을 빼면 여유가 약 226px이라 들어가지만 빠듯합니다. **모바일에서 실측 확인 필요** |

**남은 3건 중 어느 것도 구현을 막지 않습니다.** 6번만 모바일 헤더(7단계)에서 눈으로 확인하세요.
