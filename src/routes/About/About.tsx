        ...
        <div className={styles.root}>
            <div className={styles.inner}>
                ...
                <section className={styles.section}>
                     ...
                     * 됩니다. 🔴 글을 세는 단위는 `편` 입니다 — §3.4 가 2026-08-02 에
                     * 뒤집혔습니다(수량사는 세는 대상을 따름: 글 `편` / 태그 `개`).
                     */}
                    <p className={styles.section_note}>
                        글 {POSTS.length}편에 가장 많이 붙은 태그입니다. 이 블로그에 쓴
                        글에서 각 태그가 등장한 횟수입니다.
                    </p>
                    ...
                    <ul className={styles.chips}>
                        {topTags.map(tag => (
                            ...
                            <li className={styles.chip} key={tag.label}>
                                {/*
                                 * 🔴 단위는 `편` 입니다. `rankTags` 가 세는 것은 **그 태그가
                                 *    붙은 글의 편수**이고(41편의 keywords 에 중복 없음),
                                 *    바로 위 문장도 이미 `글 41편에 가장 많이 붙은 태그`
                                 *    입니다. WRITING_GUIDE §3.4 교체표가 `#React 글 14개`
                                 *    → `글 14편` 을 명시했고, "맨숫자는 유지" 예외는
                                 *    `전체 41`·`Study 30` 처럼 **단위가 없는** 표기에만
                                 *    해당합니다 — `16개` 는 맨숫자가 아닙니다.
                                 *
                                 * 맨숫자로 두지 않는 이유는 그대로입니다: `Python 16` 은
                                 * §5.3 의 ❌ 예시(`React Lv.9`)와 형태가 같아 개수가 아니라
                                 * 자기 신고 등급으로 읽힙니다.
                                 */}
                                <span aria-hidden="true">
                                    {tag.label} {tag.count}편
                                </span>
                                {/*
                                 * 화면 표기만으로는 16 이 무엇인지 음성에서 알 수 없어
                                 * 이름을 따로 줍니다. **화면과 같은 단위**여야 합니다 —
                                 * 같은 숫자를 두 단위로 말하면 어느 쪽이 사실인지 알 수
                                 * 없습니다.
                                 */}
                                <span className="sr-only">
                                    {tag.label} 태그 글 {tag.count}편
                                </span>
                            </li>
                        ))}
                    </ul>
